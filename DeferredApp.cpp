...
private:
    ...
    UINT mDeferredHeapIndexStart = 0;
...
{
    // Add +1 for screen normal map, +2 for ambient maps.,+4 for G-buffer
...
void SsaoApp::OnResize()
{
    ...
    if (mDeferredRenderer != nullptr)
    {
        mDeferredRenderer->OnResize(mClientWidth, mClientHeight);

        // Resources changed, so need to rebuild descriptors.
        mDeferredRenderer->RebuildDescriptors(mDepthStencilBuffer.Get());
    }
}
...
void SsaoApp::Draw(const GameTimer& gt)
{
    ...
    CD3DX12_GPU_DESCRIPTOR_HANDLE GBufferDescriptor(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
    GBufferDescriptor.Offset(mDeferredHeapIndexStart, mCbvSrvUavDescriptorSize);
    mCommandList->SetGraphicsRootDescriptorTable(5, GBufferDescriptor);
...
void SsaoApp::BuildRootSignature()
{
	...
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 7, 0);
...
void SsaoApp::BuildDescriptorHeaps()
{
    ...
    mDeferredHeapIndexStart = mSsaoHeapIndexStart + 5;
    mNullCubeSrvIndex = mDeferredHeapIndexStart + 4;
    ...
    md3dDevice->CreateShaderResourceView(nullptr, &srvDesc, nullSrv);
    // 1. 定义RTV起始索引常量（便于维护）
    const UINT RTV_INDEX_SWAPCHAIN_END = SwapChainBufferCount; // 2
    const UINT RTV_INDEX_SSAO_NORMAL = RTV_INDEX_SWAPCHAIN_END; // 2
    const UINT RTV_INDEX_SSAO_AMBIENT0 = RTV_INDEX_SSAO_NORMAL + 1; //3
    const UINT RTV_INDEX_SSAO_AMBIENT1 = RTV_INDEX_SSAO_AMBIENT0 + 1; //4
    const UINT RTV_INDEX_GBUFFER_START = RTV_INDEX_SSAO_AMBIENT1 + 1; //5
    ...
    mSsao->BuildDescriptors(
        ...
        GetRtv(RTV_INDEX_SSAO_NORMAL),
        mCbvSrvUavDescriptorSize,
        mRtvDescriptorSize);

    mDeferredRenderer->BuildDescriptors(
        mDepthStencilBuffer.Get(),
        GetCpuSrv(mDeferredHeapIndexStart),
        GetGpuSrv(mDeferredHeapIndexStart),
        GetRtv(RTV_INDEX_GBUFFER_START),
        mCbvSrvUavDescriptorSize,
        mRtvDescriptorSize);
    ...
    if (hr != S_OK)
    {
        // 方法1：直接打印错误码（不需要DxException）
        char errMsg[256];
        sprintf_s(errMsg, "调用BuildDescriptors后设备错误，错误码：0x%08X", hr);
...
void SsaoApp::BuildPSOs()
{
   ...
   //
    ...
    DeferredPsoDesc.DepthStencilState.DepthFunc = D3D12_COMPARISON_FUNC_LESS;
    DeferredPsoDesc.DepthStencilState.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
    DeferredPsoDesc.InputLayout = { nullptr, 0 };
    DeferredPsoDesc.VS =
    {
        reinterpret_cast<BYTE*>(mShaders["DeferredRenderingVS"]->GetBufferPointer()),
        mShaders["DeferredRenderingVS"]->GetBufferSize()
    };
    DeferredPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(mShaders["DeferredRenderingPS"]->GetBufferPointer()),
        mShaders["DeferredRenderingPS"]->GetBufferSize()
    };
...
void SsaoApp::DrawGBuffer()
{
    ...
    auto normalMap = mDeferredRenderer->NormalMap();
    auto normalMapRtv = mDeferredRenderer->NormalMapRtv();

    auto PositionMap = mDeferredRenderer->PositionMap();
    auto PositionMapRtv = mDeferredRenderer->PositionMapRtv();

    auto AlbedoMap = mDeferredRenderer->AlbedoMap();
    auto AlbedoMapRtv = mDeferredRenderer->AlbedoMapRtv();
...
CD3DX12_CPU_DESCRIPTOR_HANDLE SsaoApp::GetRtv(int index)const
{
    ...
    if (mRtvHeap == nullptr)
    {
        OutputDebugStringA("GetRtv: mRtvHeap未初始化！");
        __debugbreak();
    }
    // 验证2：索引是否超出RTV堆容量
    if (index >= mRtvHeap->GetDesc().NumDescriptors)
    {
        char errMsg[256];
        sprintf_s(errMsg, "GetRtv: 索引%d超出RTV堆容量%d！",
            index, mRtvHeap->GetDesc().NumDescriptors);
        OutputDebugStringA(errMsg);
        __debugbreak();
    }
    // 验证3：RTV步长是否有效
    if (mRtvDescriptorSize == 0)
    {
        OutputDebugStringA("GetRtv: mRtvDescriptorSize为0！");
        __debugbreak();
    }
